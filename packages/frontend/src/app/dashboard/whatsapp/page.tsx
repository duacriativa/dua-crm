...
import {
  Search, Settings, Users, Zap, Bell, BellOff, Paperclip, Smile, Mic, Send,
  Sparkles, PanelRightClose,
} from "lucide-react";

const filterTabs = ["Todas", "Não lidas", "Favoritas", "Grupos"];
...
export default function WhatsAppPage() {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] lg:h-screen flex animate-fade-in overflow-hidden">
          ...
          {conversations.map((c) => (
            <button key={c.id}
              className={`w-full flex items-center gap-3 p-3 hover:bg-muted/40 transition-colors text-left border-l-2 ${
                (c as any).active ? "bg-muted/50 border-l-primary" : "border-l-transparent"
              }`}>
      ...
      <section className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-border flex items-center gap-3 px-4 bg-card/30 shrink-0">
          ...
          <button className="p-2 rounded-lg hover:bg-muted/60 transition-colors text-muted-foreground hover:text-foreground">
            <PanelRightClose className="h-4 w-4" />
          </button>
        </header>

        <div className="flex-1 overflow-y-auto p-6 space-y-3 scrollbar-none"
          style={{ background: "radial-gradient(ellipse at top, hsl(var(--primary)/0.05), transparent 50%)" }}>
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.from === "me" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${
                ...
                {m.from === "them" && (m as any).author && (
                  <p className="text-xs font-semibold text-primary mb-0.5">{(m as any).author}</p>
                )}
      ...
      <aside className="w-72 border-l border-border bg-card/30 hidden xl:flex flex-col shrink-0">
        ...
        <div className="p-4 border-t border-border">
          <div className="w-full text-center py-2 px-4 rounded-xl bg-primary/10 text-primary border border-primary/20 text-sm font-medium">
