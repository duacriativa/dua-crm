...
const MOCK_CONVERSATIONS: Conversation[] = [
  {
    ...
    contact: { id: "c1", name: "Ana Lima", phone: "+5511999990001", tags: ["VIP", "Loja SP"] },
    lastMessage: "Oi! Quero saber sobre a nova coleção 🌸",
    unreadCount: 3,
    assigneeName: "Daniel",
  },
  {
    ...
    channel: "WHATSAPP",
    status: "OPEN",
    externalId: "+5521988880002",
    createdAt: new Date(Date.now() - 7200000).toISOString(),
    updatedAt: new Date(Date.now() - 1800000).toISOString(),
    contact: { id: "c2", name: "Beatriz Souza", phone: "+5521988880002", tags: ["Prospect"] },
    lastMessage: "Qual o prazo de entrega para o RJ?",
    unreadCount: 1,
    assigneeName: "Daniel",
  },
  {
    ...
    channel: "INSTAGRAM",
    status: "PENDING",
    externalId: "@mari.moda",
    createdAt: new Date(Date.now() - 86400000).toISOString(),
    updatedAt: new Date(Date.now() - 3600000).toISOString(),
    contact: { id: "c3", name: "Mariana Costa", tags: ["Influencer"] },
    lastMessage: "Adorei o vestido da foto! Tem na cor azul?",
    unreadCount: 0,
  },
  {
    id: "4",
    channel: "WHATSAPP",
    status: "RESOLVED",
    externalId: "+5531977770003",
    createdAt: new Date(Date.now() - 172800000).toISOString(),
    updatedAt: new Date(Date.now() - 86400000).toISOString(),
    contact: { id: "c4", name: "Carla Mendes", phone: "+5531977770003" },
...
const MOCK_MESSAGES: Record<string, Message[]> = {
  ...
  "3": [
    { id: "m7", content: "Adorei o vestido da foto! Tem na cor azul?", direction: "INBOUND", type: "TEXT", createdAt: new Date(Date.now() - 3600000).toISOString() },
  ],
  "4": [
...
export default function ConversasPage() {
  ...
  const sendMessage = async () => {
    ...
    setConversations((prev) =>
      prev.map((c) =>
        c.id === selected.id ? { ...c, lastMessage: msg.content, updatedAt: msg.createdAt } : c
      )
    );
    setNewMessage("");
    setSending(false);
  ...
  return (
    <div className="flex h-full overflow-hidden">
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((conv) => (
              <button
                ...
                className={`w-full flex items-start gap-3 px-4 py-3.5 text-left border-b border-gray-50 hover:bg-gray-50 transition-colors ${
                  selected?.id === conv.id ? "bg-brand-50 border-l-2 border-l-brand-500" : ""
                }`}
              >
                {/* Avatar */}
                <div className="relative shrink-0">
                  <div className="w-10 h-10 rounded-full bg-brand-100 text-brand-600 font-bold text-sm flex items-center justify-center">
          ...
          <div className="bg-white border-t border-gray-200 px-4 py-3">
            <div className="flex items-end gap-2">
              ...
              <textarea
                ...
                rows={1}
                className="flex-1 resize-none bg-gray-50 border border-gray-200 rounded-2xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent max-h-32"
                style={{ minHeight: "42px" }}
              />
              <button
                onClick={sendMessage}
                disabled={!newMessage.trim() || sending}
                className="p-2.5 bg-brand-600 text-white rounded-2xl hover:bg-brand-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                <Send className="w-4 h-4" />
              </button>
            </div>
            <p className="text-[10px] text-gray-400 mt-1.5 px-2">Enter para enviar · Shift+Enter para nova linha</p>
          </div>
        </div>
      ) : (
      ...
      {/* ── Col 3: Info do contato ── */}
      {selected && (
        <aside className="w-72 flex flex-col border-l border-gray-200 bg-white shrink-0 overflow-y-auto">
          {/* Contact header */}
          <div className="p-5 border-b border-gray-100 text-center">
            ...
            {selected.contact.phone && (
              <p className="text-xs text-gray-400 mt-0.5">{selected.contact.phone}</p>
            )}
            {selected.contact.email && (
              <p className="text-xs text-gray-400">{selected.contact.email}</p>
            )}
            <div className="flex items-center justify-center gap-2 mt-3">
              <button className="flex-1 py-1.5 text-xs font-semibold text-brand-600 border border-brand-200 rounded-lg hover:bg-brand-50 transition-colors">
                Ver Perfil
              </button>
          ...
          <div className="p-4 border-b border-gray-100">
            ...
            <div className="flex flex-wrap gap-1.5">
              {selected.contact.tags?.map((tag) => (
                <span key={tag} className="text-xs bg-brand-50 text-brand-700 border border-brand-100 px-2 py-1 rounded-full font-medium">
                  {tag}
                </span>
              ))}
              <button className="text-xs text-gray-400 border border-dashed border-gray-300 px-2 py-1 rounded-full hover:border-brand-400 hover:text-brand-500 transition-colors">
                + Add tag
              </button>
            </div>
          </div>
          ...
          <div className="p-4 border-b border-gray-100 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                {channelIcon(selected.channel)}
                <span className="text-xs font-medium text-gray-700">{selected.channel}</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-medium text-gray-700">{selected.assigneeName || "—"}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-400">Iniciada em</span>
          ...
          <div className="p-4 space-y-2">
            ...
            <button className="w-full text-left text-sm px-3 py-2.5 rounded-xl bg-green-50 text-green-700 font-medium hover:bg-green-100 transition-colors flex items-center gap-2">
              <CheckCheck className="w-4 h-4" />
              Marcar como resolvida
            </button>
            <button className="w-full text-left text-sm px-3 py-2.5 rounded-xl bg-yellow-50 text-yellow-700 font-medium hover:bg-yellow-100 transition-colors flex items-center gap-2">
              <Clock className="w-4 h-4" />
              Marcar como pendente
            </button>
            <button className="w-full text-left text-sm px-3 py-2.5 rounded-xl bg-gray-50 text-gray-600 font-medium hover:bg-gray-100 transition-colors flex items-center gap-2">
              <Star className="w-4 h-4" />
              Favoritar contato
            </button>
            <button className="w-full text-left text-sm px-3 py-2.5 rounded-xl bg-red-50 text-red-600 font-medium hover:bg-red-100 transition-colors flex items-center gap-2">
              <Archive className="w-4 h-4" />
              Arquivar conversa
            </button>
          </div>
